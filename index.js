...
function getArtistByIndex(array, num1) {
  return `the artist at index ${num1} is ${artists[num1].name}`
}
...
function get20s(array2) {
  let newArray = []
  let newSplice = ""

  for (let i = 0; i < array2.length; i++) {
  newSplice = artists[i].years
  let spliceFun = newSplice.split(" - ")

  if (spliceFun[0] > 1899 && spliceFun[0] < 2000 && spliceFun[0] > 1899 && spliceFun[0] < 2000)
...
function removeArtist(getArray, num2) {
  getArray.splice(num2, 1)
...
function lotsOfArt(getArray3) {
  let newArray = []

  for (let i=0; i < getArray3.length; i++) {
    if (getArray3[i].paintings > 100) {
      newArray.push(getArray3[i].name)
    }
  }
  return newArray
}
...
*/
function artistInfo(getArray4, artName){
  for (let i = 0; i < getArray4.length; i++) {
    if (getArray4[i].name === artName) {
      return [getArray4[i].bio]
    }
  }
}
...
function artistByCountry(getArray5, nationality){
  let newArray2 = []
  for (let i = 0; i < getArray5.length; i++) {
    if (getArray5[i].nationality === nationality) {
      newArray2.push(getArray5[i].name)
    }
  }
  return newArray2
}
